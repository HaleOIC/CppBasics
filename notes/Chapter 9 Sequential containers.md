...
- `swap`操作除了对于`string`之外，指向容器的迭代器、引用和指针在`swap`操作之后都不会失效，而`string`将失效

### 关系运算符

两个容器比较实际上是进行元素的逐对比较，并且**运算对象必须是相同类型的容器**：

- 如果两个容器具有相同大小并且元素都两两对应相等，则两个容器相等；
- 如果大小不同，但较小的容器中的元素都等于较大容器中的对应元素，则较小容器小于较大容器
- 否则，比较结果取决于第一个不相等元素的比较结果
...
- 当我们用一个对象去初始化 容器或者将对象插入到容器时，实际上放入的是对象的拷贝。
- `emplace`开头的函数是新标准引入的，这些操作是**构造而不是拷贝元素。**
...
| 操作                       | 解释                                                         |
| -------------------------- | ------------------------------------------------------------ |
| `string s(cp, n)`          | `s`是`cp`指向的数组中前`n`个字符的拷贝，此数组至少包括n个字符 |
